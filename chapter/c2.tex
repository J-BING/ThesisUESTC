\chapter{视觉问答数据集}
从LeCun的MNIST数据集\citing{lecun1998mnist}到如今大量的人工智能任务的数据集，优质的数据集已经成为研究工作的重要部分，是监督学习模型的训练基础。优秀的数据集需要具有足够大的容量\citing{sun2017revisiting}、规范友好数据格式、较小的数据偏见等特点。
...
正如表格所示，KB-VQA和FVQA属于“基于知识的数据集”，其他均属于“基于视觉的数据集”。本章我们将从这两个方面概述视觉问答数据集，并分析各自的特点，最后从中挑选出本文使用的实验数据集。
...
为了衡量模型在识别类和推理类两类问题的表现，我们采用了VQA2.0数据集、KB-VQA和FVQA三个数据集，其中VQA2.0数据集用于训练N-KBSN模型，而KB-VQA和FVQA则用于训练KBSN模型，最后混合三种数据集用于测试模型的表现。
